// Мультфильм "Природа в городской среде"  |
...
void DrawDog           (int x, int y, double sizeX, double sizeY, int TailShift,
...
while (x < 600)
  ...
  DrawTree          (200, 300, 1  , 1  , 8, RGB(0, 0, 0), RGB(0, 0, 0), RGB(0, 0, 0));
  DrawTree          (300, 300, 0.8, 0.8, 7, RGB(0, 0, 0), RGB(0, 0, 0), RGB(0, 0, 0));
  DrawTree          (550, 250, 0.5, 0.5, 6, RGB(0, 0, 0), RGB(0, 0, 0), RGB(0, 0, 0));
...
while (x < 200)
  ...
  DrawTree (200, 300, 1  , 1  , 8, RGB(255 - x, 0, 0), RGB(255 - x, 0, 0), RGB(255 - x, 0, 0));
  DrawTree (300, 300, 0.8, 0.8, 7, RGB(255 - x, 0, 0), RGB(255 - x, 0, 0), RGB(255 - x, 0, 0));
  DrawTree (550, 250, 0.5, 0.5, 6, RGB(255 - x, 0, 0), RGB(255 - x, 0, 0), RGB(255 - x, 0, 0));
...
while (x < 80)
  ...
  DrawTree (200, 300, 1  , 1  , 8, RGB(255, 0, 0), RGB(255, 0, 0), RGB(255, 0, 0));
  DrawTree (300, 300, 0.8, 0.8, 7, RGB(255, 0, 0), RGB(255, 0, 0), RGB(255, 0, 0));
  DrawTree (550, 250, 0.5, 0.5, 6, RGB(255, 0, 0), RGB(255, 0, 0), RGB(255, 0, 0));
...
while (x < 400)
  ...
  DrawDog  (700 - 4 * x, 470,  1, 1, x %  2, TX_BLACK, TX_WHITE, TX_GREY);
  DrawDog  (750 - 2 * x, 480,  1, 1, x % 10, TX_WHITE, TX_BLUE , TX_BLUE);
  DrawDog  ( 50 + 2 * x, 380, -1, 1, x %  7, TX_WHITE, TX_GREY , TX_RED );
...
while (x < 50)
      ...
      DrawTree (200, 300,  1  , 1  ,  8,
                RGB(200 + x, 180 - x, 0), RGB(200 + x, 180 - x, 0), RGB(200 + x, 180 - x, 0));
      DrawTree (300, 300,  0.8, 0.8,  7,
                RGB(200 + x, 180 - x, 0), RGB(200 + x, 180 - x, 0), RGB(200 + x, 180 - x, 0));
      DrawTree (550, 250,  0.5, 0.5,  6,
                RGB(200 + x, 180 - x, 0), RGB(200 + x, 180 - x, 0), RGB(200 + x, 180 - x, 0));
...
while (t < 600)
  ...
  DrawBird       (550 - t, 250, 1, 1, t % 5, TX_WHITE, TX_ORANGE, TX_WHITE);
  DrawBird       (570 - t, 200, 1, 1, t % 5, TX_WHITE, TX_ORANGE, TX_WHITE);
  DrawBird       (590 - t, 150, 1, 1, t % 5, TX_WHITE, TX_ORANGE, TX_WHITE);
  t ++;
  txSleep (5);
  }
}
...
while (x < 40)
      ...
      DrawTree (200, 300, 0.025  * x, 0.025  * x,  8,
                RGB(0, 100 +  2  * x, 0), RGB(0, 100 + 2 * x, 0), RGB(0, 100 + 2 * x, 0));
      DrawTree (300, 300, 0.02   * x, 0.02   * x,  7,
                RGB(0, 100 + 2   * x, 0), RGB(0, 100 + 2 * x, 0), RGB(0, 100 + 2 * x, 0));
      DrawTree (550, 250, 0.0125 * x, 0.0125 * x,  6,
                RGB(0, 100 + 2   * x, 0), RGB(0, 100 + 2 * x, 0), RGB(0, 100 + 2 * x, 0));
      x ++;
      txSleep (100);
      txClear ();
...
POINT dress [] = {{x, y}, {x - 25 * sizeX, y + 88 * sizeY}, {x + 25 * sizeX, y + 88 * sizeY}, {x, y}};
txPolygon (dress, 3);
}
...
txRectangle (x, y - 150 * sizeY, x + 25 * sizeX, y);

txSetColor     (TX_GREEN);
txSetFillColor (TX_GREEN);
POINT tree [] = {{x - 25 * sizeX, y - 150 * sizeY}, {x - 50 * sizeX, y - 200 * sizeY},
                 {x + 12 * sizeX, y - 350 * sizeY}, {x + 75 * sizeX, y - 200 * sizeY},
                 {x + 50 * sizeX, y - 150 * sizeY}, {x - 25 * sizeX, y - 150 * sizeY}};
txPolygon (tree, 5);

txSetColor     (colorFruit1);
txSetFillColor (colorFruit1);
txCircle (x + 25 * sizeX, y - 275 * sizeY, DFruit * sizeY);
txCircle (x,              y - 225 * sizeY, DFruit * sizeY);
txCircle (x + 50 * sizeX, y - 200 * sizeY, DFruit * sizeY);

txSetColor     (colorFruit2);
txSetFillColor (colorFruit2);
txCircle (x + 25 * sizeX, y - 200 * sizeY, DFruit * sizeY);
txCircle (x             , y - 250 * sizeY, DFruit * sizeY);

txSetColor     (colorFruit3);
txSetFillColor (colorFruit3);
txCircle (x - 25 * sizeX, y - 225 * sizeY, DFruit * sizeY);
txCircle (x             , y - 175 * sizeY, DFruit * sizeY);
}
...
POINT body [] = {{x, y}, {x + 25 * sizeX, y - 5 * sizeY}, {x + 10 * sizeX, y + 5 * sizeY}, {x, y}};
txPolygon (body, 4);
}
...
POINT neck [] = {{x +  5 * sizeX, y - 10 * sizeY},  {x             , y -  5 * sizeY}, {x -  5 * sizeX, y - 10 * sizeY},
                 {x - 15 * sizeX, y - 10 * sizeY},  {x - 15 * sizeX, y - 13 * sizeY}, {x - 10 * sizeX, y - 15 * sizeY},
                 {x             , y - 25 * sizeY},  {x             , y - 15 * sizeY}, {x +  5 * sizeX, y - 10 * sizeY}};
txPolygon (neck, 9);
...
POINT lapa [] = {{x + 15 * sizeX, y +  5 * sizeY}, {x + 15 * sizeX, y + 10 * sizeY}, {x + 10 * sizeX, y + 15 * sizeY},
                 {x + 15 * sizeX, y + 15 * sizeY}, {x + 20 * sizeX, y + 10 * sizeY}, {x + 20 * sizeX, y +  5 * sizeY},
                 {x + 15 * sizeX, y +  5 * sizeY}};
txPolygon (lapa, 7);
}
